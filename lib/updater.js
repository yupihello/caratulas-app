...
function applyPendingUpdate() {
  ...
  // Case 1: We ARE the _new.exe (just launched by the old instance).
  // Copy ourselves over the original exe name, then delete ourselves later.
  if (base.endsWith('_new')) {
    const originalBase = base.replace(/_new$/, '');
    const originalPath = path.join(dir, originalBase + ext);
    log(`STARTUP: running as _new (${exePath}), copying over ${originalPath}`);
    try {
      fs.copyFileSync(exePath, originalPath);
      log(`STARTUP: original overwritten with _new successfully`);
      // We can't delete ourselves while running — mark for cleanup on next normal launch
    } catch (err) {
      log(`STARTUP: copy to original failed: ${err.code} ${err.message}`);
    }
    return;
  }

  // Case 2: Normal launch of CaratulasApp.exe. If _new exists, delete it (leftover from case 1).
  ...
  if (fs.existsSync(newPath)) {
    try {
      ...
      log(`STARTUP: cleaned up leftover _new`);
    } catch (err) {
      log(`STARTUP: cleanup _new failed: ${err.code} ${err.message}`);
    }
  }
}
