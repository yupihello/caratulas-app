...
function applyPendingUpdate() {
  ...
  try {
    // Overwrite current exe with new version (exe is not locked yet on fresh start)
    fs.copyFileSync(newPath, exePath);
    fs.unlinkSync(newPath);
    log(`STARTUP: updated exe overwritten, _new deleted`);
  } catch (err) {
    log(`STARTUP: overwrite failed: ${err.code} ${err.message}`);
  }
}
...
async function downloadAndApplyUpdate(updateInfo, mainWindow) {
  ...
  log(`DOWNLOAD COMPLETE: ${newPath} (${fs.statSync(newPath).size} bytes). Relaunching.`);

  // Relaunch the same exe — it will apply the update on startup via applyPendingUpdate()
  const exePath = getPortableExePath();
