...
async function applyPendingUpdate() {
  ...
  // Retry copy up to 15 times (1s apart) — old instance may still be dying
  for (let attempt = 1; attempt <= 15; attempt++) {
    try {
      fs.copyFileSync(newPath, exePath);
      fs.unlinkSync(newPath);
      log(`STARTUP: exe overwritten on attempt ${attempt}, _new deleted`);
      return;
    } catch (err) {
      log(`STARTUP: attempt ${attempt} failed: ${err.code}`);
      if (attempt < 15) await new Promise(r => setTimeout(r, 1000));
    }
  }
  log(`STARTUP: gave up after 15 attempts, _new remains for next launch`);
}
...
async function downloadAndApplyUpdate(updateInfo, mainWindow) {
  ...
  // Relaunch: open the exe via shell, then quit.
  // The new instance's applyPendingUpdate() will retry for up to 15s
  // waiting for this process to die and release the exe lock.
  const exePath = getPortableExePath();
  log(`RELAUNCH: ${exePath}`);
